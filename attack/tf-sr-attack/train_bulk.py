...
def main(unused_argv):
  ...
  while (len(image_indices) > 0):
    ...
    for image_index in image_indices:
      ...
      tmp = numpy.zeros([input_image.shape[0]*4, input_image.shape[1]*4, input_image.shape[2]])
      tmp[0:truth_image.shape[0], 0:truth_image.shape[1], :] = truth_image
      truth_image = tmp
  ...
  folder = "../data/test/B100_attack_"+ os.path.splitext(os.path.split(FLAGS.sr_config_path)[-1])[0] +"_"+str(FLAGS.train_path).split("/")[-1]
  lr_folder = folder + "_LR/4"
  if not os.path.exists(folder):
    os.mkdir(folder)
  if not os.path.exists(lr_folder):
    os.makedirs(lr_folder)
  import shutil
  base_folder = os.path.join(FLAGS.train_path, "attacked")
  img_list = os.listdir(base_folder)
  for img in img_list:
    shutil.copy(os.path.join(base_folder, img), lr_folder)
  HR_img = os.listdir("./test/BSDS100")
  for img in HR_img:
    shutil.copy(os.path.join("./test/BSDS100/", img), folder)
