...
class Tester():
        ...
        for dataset in self.sys_conf.test_dataset:
            ...
            for scale in self.sys_conf.scale_factor:
                ...
                for i in range(1, total_model_num + 1):
                    ...
                    #     for k in para.keys():
                    #         if "module." in k:
                    #             para_new[k.replace("module.", "")] = para[k]
                    #     net.load_state_dict(para_new)
                    net.to(self.sys_conf.device_in_prog)
                    if self.sys_conf.model_mode == "post":
                        if self.sys_conf.scale_pos == "init":
                            psnr, ssim, lpips = self.__test_pre_or_init(net, test_data.get_loader(), self.is_normal, self.sys_conf.patch)
    ...
    def show(self):
        ...
        utils.log("Use self ensemble: " + str(self.sys_conf.self_ensemble), self.sys_conf.model_name)
    ...
    def __test_pre_or_init(self, net, loader, is_normal, test_patch):
        ...
        for _, data in enumerate(loader):
            ...
            if test_patch == None:
                lr, hr = data
                if self.sys_conf.self_ensemble:
                    sr = self.forward_x8(lr.to(self.sys_conf.device_in_prog), net.forward)
                else:
                    sr = net(lr.to(self.sys_conf.device_in_prog))
                if isinstance(sr, tuple) or isinstance(sr, list):
                    sr = sr[0]
                sr = sr.permute(0, 2, 3, 1).squeeze(0).cpu()
            else:
                lr, hr, shape = data
                ...
                for n in range(b):
                    lr_patch = lr[n, :, :, :]
                    lr_patch = lr_patch.unsqueeze(0)
                    # if net.__class__.__name__ == "FDSR":
                    #     _, sr_patch = net(lr_patch.to(self.sys_conf.device_in_prog))
                    # else:
                    if self.sys_conf.self_ensemble:
                        sr_patch = self.forward_x8(lr_patch.to(self.sys_conf.device_in_prog), net.forward)
                    else:
                        sr_patch = net(lr_patch.to(self.sys_conf.device_in_prog))
                    if isinstance(sr_patch, tuple) or isinstance(sr_patch, list):
                        sr_patch = sr_patch[0]
                    sr_patch = sr_patch.cpu()
                    w_now = n % w_n
                    h_now = math.ceil((n - w_now)/w_n)
    ...
    def forward_x8(self, x, forward_function):
        ...
        sr_list = [forward_function(aug.to(x.device)) for aug in lr_list]
        for i in range(len(sr_list)):
            if isinstance(sr_list[i], tuple) or isinstance(sr_list[i], list):
                sr_list[i] = sr_list[i][0]
            sr_list[i] = sr_list[i].cpu()
            if i > 3:
        ...
        output_cat = torch.cat(sr_list, dim=0).to(x.device)
