...
class SR_dataset_RGB(Dataset):
    def __init__(self, HR_folder, LR_folder, scale, patch_size, opts, batchsize, train=True, is_post=True, normal=False, is_real=False, test_patch=None, n=1):
        ...
        if self.opts["repeat_factor"] == 0:
            ...
            self.repeat = int(self.opts["repeat_factor"] / (len(self.lr_img)/batchsize) * n)
    ...
    def get_idx(self, idx):
        if self.train:
            return idx % len(self.lr_img)
        else:
            return idx
    def __len__(self):
        if self.train:
            return len(self.lr_img) * self.repeat
        else:
    ...
    def __getitem__(self, index):
        if self.combine:
            ...
            hr_ext = os.path.splitext(self.hr_img[0])[1]
            lr_img_name = os.path.splitext(lr_name)[0]
            hr_img_name = lr_img_name.split("_")[:-1]
            hr_img_new = []
            for name_split in hr_img_name:
                hr_img_new.append(name_split+"_")
            hr_name = "".join(hr_img_new)[:-1] + hr_ext
        else:
            index = self.get_idx(index)
...
class SR_dataset_Y(SR_dataset_RGB):
    ...
    def __getitem__(self, index):
        if self.combine:
            ...
            hr_ext = os.path.splitext(self.hr_img[0])[1]
            lr_img_name = os.path.splitext(lr_name)[0]
            hr_img_name = lr_img_name.split("_")[0]
            hr_name = hr_img_name + hr_ext
        else:
            index = self.get_idx(index)
            hr_name = self.hr_img[index]
            lr_name = self.lr_img[index]
