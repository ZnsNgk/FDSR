...
from FDSR_RK2 import FDSR

def main(model_name):
    if model_name == "FDSR_DS":
    # q = 32
        para_file = torch.load("x2_wFDL.pth", map_location=torch.device("cpu"))
    elif model_name == "FDSR":
        para_file = torch.load("x2.pth", map_location=torch.device("cpu"))
    else:
        raise NameError("Unknown model name")
    model = FDSR(2, freq_c=32, c=64, mode="ideal")
